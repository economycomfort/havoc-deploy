...
resource "aws_eip" "campaign_server_eip" {
  count      = var.enable_domain_name ? 1 : 0
  instance   = aws_instance.campaign_server.id[count.index]
  vpc        = true
...
resource "aws_ebs_volume" "campaign_server_volume" {
  count             = var.enable_domain_name ? 1 : 0
  availability_zone = aws_instance.campaign_server.availability_zone[count.index]
  type              = "gp2"
  size              = 30
}
...
resource "aws_volume_attachment" "campaign_server_volume_attachment" {
  count       = var.enable_domain_name ? 1 : 0
  device_name = "/dev/sdh"
  instance_id = aws_instance.campaign_server.id[count.index]
  volume_id   = aws_ebs_volume.campaign_server_volume.id[count.index]
}