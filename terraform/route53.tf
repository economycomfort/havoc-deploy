...
resource "aws_route53_record" "campaign_api_cert_validation" {
  ...
  name            = aws_acm_certificate.api_gateway_cert[count.index].domain_validation_options[1].resource_record_name
  records         = [aws_acm_certificate.api_gateway_cert[count.index].domain_validation_options[1].resource_record_value]
  ttl             = 60
  type            = aws_acm_certificate.api_gateway_cert[count.index].domain_validation_options[1].resource_record_type
  zone_id         = var.hosted_zone
}
